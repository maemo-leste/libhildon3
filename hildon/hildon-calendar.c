 ...
 * operations.
 *
 * #HildonCalendar is a widget that displays a Gregorian calendar, one month
 * at a time. It can be created with hildon_calendar_new().
 *
 * The month and year currently displayed can be altered with
 * hildon_calendar_select_month(). The exact day can be selected from the
 * displayed month using hildon_calendar_select_day().
 *
 * To place a visual marker on a particular day, use hildon_calendar_mark_day()
 * and to remove the marker, hildon_calendar_unmark_day(). Alternative, all
 * marks can be cleared with hildon_calendar_clear_marks().
 *
 * The way in which the calendar itself is displayed can be altered using
 * hildon_calendar_set_display_options().
 *
 * The selected date can be retrieved from a #HildonCalendar using
...
// HACK to remove undefined functions:
...
static guint hildon_calendar_signals[LAST_SIGNAL] = { 0 };
...
struct _HildonCalendarPrivate
{
  ...
  HildonCalendarDetailFunc detail_func;
...
static void hildon_calendar_finalize     (GObject      *calendar);
static void hildon_calendar_destroy      (GtkWidget    *widget);
static void hildon_calendar_set_property (GObject      *object,
                                       guint         prop_id,
                                       const GValue *value,
                                       GParamSpec   *pspec);
static void hildon_calendar_get_property (GObject      *object,
                                       guint         prop_id,
                                       GValue       *value,
                                       GParamSpec   *pspec);

static void     hildon_calendar_realize        (GtkWidget        *widget);
static void     hildon_calendar_unrealize      (GtkWidget        *widget);
static void     hildon_calendar_map            (GtkWidget        *widget);
static void     hildon_calendar_unmap          (GtkWidget        *widget);
static void     hildon_calendar_get_preferred_width  (GtkWidget   *widget,
                                                   gint        *minimum,
                                                   gint        *natural);
static void     hildon_calendar_get_preferred_height (GtkWidget   *widget,
                                                   gint        *minimum,
                                                   gint        *natural);
static void     hildon_calendar_size_allocate  (GtkWidget        *widget,
                                             GtkAllocation    *allocation);
static gboolean hildon_calendar_draw           (GtkWidget        *widget,
                                             cairo_t          *cr);
static gboolean hildon_calendar_button_press   (GtkWidget        *widget,
                                             GdkEventButton   *event);
static gboolean hildon_calendar_button_release (GtkWidget        *widget,
                                             GdkEventButton   *event);
static gboolean hildon_calendar_motion_notify  (GtkWidget        *widget,
                                             GdkEventMotion   *event);
static gboolean hildon_calendar_enter_notify   (GtkWidget        *widget,
                                             GdkEventCrossing *event);
static gboolean hildon_calendar_leave_notify   (GtkWidget        *widget,
                                             GdkEventCrossing *event);
static gboolean hildon_calendar_scroll         (GtkWidget        *widget,
                                             GdkEventScroll   *event);
static gboolean hildon_calendar_key_press      (GtkWidget        *widget,
                                             GdkEventKey      *event);
static gboolean hildon_calendar_focus_out      (GtkWidget        *widget,
                                             GdkEventFocus    *event);
static void     hildon_calendar_grab_notify    (GtkWidget        *widget,
                                             gboolean          was_grabbed);
static void     hildon_calendar_state_flags_changed  (GtkWidget     *widget,
                                                   GtkStateFlags  previous_state);
static gboolean hildon_calendar_query_tooltip  (GtkWidget        *widget,
                                             gint              x,
                                             gint              y,
                                             gboolean          keyboard_mode,
                                             GtkTooltip       *tooltip);

static void     hildon_calendar_drag_data_get      (GtkWidget        *widget,
                                                 GdkDragContext   *context,
                                                 GtkSelectionData *selection_data,
                                                 guint             info,
                                                 guint             time);
static void     hildon_calendar_drag_data_received (GtkWidget        *widget,
                                                 GdkDragContext   *context,
                                                 gint              x,
                                                 gint              y,
                                                 GtkSelectionData *selection_data,
                                                 guint             info,
                                                 guint             time);
static gboolean hildon_calendar_drag_motion        (GtkWidget        *widget,
                                                 GdkDragContext   *context,
                                                 gint              x,
                                                 gint              y,
                                                 guint             time);
static void     hildon_calendar_drag_leave         (GtkWidget        *widget,
                                                 GdkDragContext   *context,
                                                 guint             time);
static gboolean hildon_calendar_drag_drop          (GtkWidget        *widget,
                                                 GdkDragContext   *context,
                                                 gint              x,
                                                 gint              y,
                                                 guint             time);
...
static void calendar_start_spinning (HildonCalendar *calendar,
                                     gint         click_child);
static void calendar_stop_spinning  (HildonCalendar *calendar);

static void calendar_invalidate_day     (HildonCalendar *widget,
                                         gint       row,
                                         gint       col);
static void calendar_invalidate_day_num (HildonCalendar *widget,
                                         gint       day);
static void calendar_invalidate_arrow   (HildonCalendar *widget,
                                         guint      arrow);

static void calendar_compute_days      (HildonCalendar *calendar);
static gint calendar_get_xsep          (HildonCalendar *calendar);
static gint calendar_get_ysep          (HildonCalendar *calendar);
static gint calendar_get_inner_border  (HildonCalendar *calendar);

static char    *default_abbreviated_dayname[7];
static char    *default_monthname[12];

G_DEFINE_TYPE_WITH_PRIVATE (HildonCalendar, hildon_calendar, GTK_TYPE_WIDGET)
...
hildon_calendar_class_init (HildonCalendarClass *class)
{
  ...
  widget_class->destroy = hildon_calendar_destroy;
  widget_class->realize = hildon_calendar_realize;
  widget_class->unrealize = hildon_calendar_unrealize;
  widget_class->map = hildon_calendar_map;
  widget_class->unmap = hildon_calendar_unmap;
  widget_class->draw = hildon_calendar_draw;
  widget_class->get_preferred_width = hildon_calendar_get_preferred_width;
  widget_class->get_preferred_height = hildon_calendar_get_preferred_height;
  ...
  widget_class->state_flags_changed = hildon_calendar_state_flags_changed;
  widget_class->grab_notify = hildon_calendar_grab_notify;
  widget_class->focus_out_event = hildon_calendar_focus_out;
  widget_class->query_tooltip = hildon_calendar_query_tooltip;
...
/**
 * HildonCalendar:detail-width-chars:
 *
 * Width of a detail cell, in characters.
 * A value of 0 allows any width. See hildon_calendar_set_detail_func().
 *
 * Since: 2.14
 */
  g_object_class_install_property (gobject_class,
...
/**
 * HildonCalendar:detail-height-rows:
 *
 ...
 */
  ...
  /**
   * HildonCalendar:horizontal-separation:
   *
  ...
  /**
   * HildonCalendar::day-selected-double-click:
   * @calendar: the object which received the signal.
   *
...
hildon_calendar_init (HildonCalendar *calendar)
{
  ...
  priv = calendar->priv = hildon_calendar_get_instance_private (calendar);
  ...
  priv->display_flags = (HILDON_CALENDAR_SHOW_HEADING |
                             HILDON_CALENDAR_SHOW_DAY_NAMES |
                             HILDON_CALENDAR_SHOW_DETAILS);
...
static void
calendar_queue_refresh (HildonCalendar *calendar)
{
  HildonCalendarPrivate *priv = calendar->priv;

  if (!(priv->detail_func) ||
      !(priv->display_flags & HILDON_CALENDAR_SHOW_DETAILS) ||
...
static void
calendar_set_month_next (HildonCalendar *calendar)
{
  gint month_len;
  HildonCalendarPrivate *priv = calendar->priv;

  if (priv->display_flags & HILDON_CALENDAR_NO_MONTH_CHANGE)
    return;

  if (priv->month == 11)
    {
      if (!priv->max_year || priv->year < priv->max_year)
      {
        priv->month = 0;
        priv->year++;
      }
  ...
  if (month_len < priv->selected_day)
    ...
    hildon_calendar_select_day (calendar, priv->selected_day);

  calendar_queue_refresh (calendar);
}

static void
calendar_set_year_prev (HildonCalendar *calendar)
{
  HildonCalendarPrivate *priv = calendar->priv;
  gint month_len;

  if (!priv->min_year || priv->min_year < priv->year)
    priv->year--;
  calendar_compute_days (calendar);
  g_signal_emit (calendar,
                 hildon_calendar_signals[PREV_YEAR_SIGNAL],
...
static void
calendar_set_year_next (HildonCalendar *calendar)
{
  HildonCalendarPrivate *priv = calendar->priv;
  gint month_len;

  if (!priv->max_year || priv->max_year > priv->year)
    priv->year++;
  calendar_compute_days (calendar);
  g_signal_emit (calendar,
                 hildon_calendar_signals[NEXT_YEAR_SIGNAL],
...
static void
calendar_compute_days (HildonCalendar *calendar)
{
  HildonCalendarPrivate *priv = calendar->priv;
  gint month;
  gint year;
...
static void
calendar_select_and_focus_day (HildonCalendar *calendar,
                               guint        day)
{
  HildonCalendarPrivate *priv = calendar->priv;
...
static gint
calendar_row_height (HildonCalendar *calendar)
{
  HildonCalendarPrivate *priv = calendar->priv;

  return (priv->main_h - CALENDAR_MARGIN
          - ((priv->display_flags & HILDON_CALENDAR_SHOW_DAY_NAMES)
             ? calendar_get_ysep (calendar) : CALENDAR_MARGIN)) / 6;
}

static void
get_component_paddings (HildonCalendar *calendar,
                        GtkBorder   *padding,
...
static gint
calendar_left_x_for_column (HildonCalendar *calendar,
                            gint         column)
{
  HildonCalendarPrivate *priv = calendar->priv;
  gint width;
  ...
  if (priv->display_flags & HILDON_CALENDAR_SHOW_WEEK_NUMBERS)
...
static gint
calendar_column_from_x (HildonCalendar *calendar,
                        gint         event_x)
{
  gint c, column;
  gint x_left, x_right;
...
static gint
calendar_top_y_for_row (HildonCalendar *calendar,
                        gint         row)
{
  HildonCalendarPrivate *priv = calendar->priv;
  GtkBorder padding;
...
static gint
calendar_row_from_y (HildonCalendar *calendar,
                     gint         event_y)
{
  gint r, row;
  gint height;
...
static void
calendar_arrow_rectangle (HildonCalendar  *calendar,
                          guint         arrow,
                          GdkRectangle *rect)
{
  GtkWidget *widget = GTK_WIDGET (calendar);
...
static void
calendar_day_rectangle (HildonCalendar  *calendar,
                        gint          row,
                        gint          col,
                        GdkRectangle *rect)
{
...
static void
calendar_set_month_prev (HildonCalendar *calendar)
{
  HildonCalendarPrivate *priv = calendar->priv;
  gint month_len;
  ...
  if (priv->month == 0)
    {
      if (!priv->min_year || priv->year > priv->min_year)
      {
        priv->month = 11;
        priv->year--;
      }
...
{
  G_OBJECT_CLASS (hildon_calendar_parent_class)->finalize (object);
}

static void
hildon_calendar_destroy (GtkWidget *widget)
{
  HildonCalendarPrivate *priv = HILDON_CALENDAR (widget)->priv;

  calendar_stop_spinning (HILDON_CALENDAR (widget));
  ...
  GTK_WIDGET_CLASS (hildon_calendar_parent_class)->destroy (widget);
}

static gboolean
calendar_set_display_option (HildonCalendar              *calendar,
...
static gboolean
calendar_get_display_option (HildonCalendar              *calendar,
                             HildonCalendarDisplayOptions flag)
{
  HildonCalendarPrivate *priv = calendar->priv;
...
hildon_calendar_set_property (GObject      *object,
                           ...
                           GParamSpec   *pspec)
{
  HildonCalendar *calendar = HILDON_CALENDAR (object);
  HildonCalendarPrivate *priv = calendar->priv;
  gint val;
  ...
  switch (prop_id)
    ...
    case PROP_DETAIL_WIDTH_CHARS:
      hildon_calendar_set_detail_width_chars (calendar,
                                           g_value_get_int (value));
      break;
    case PROP_DETAIL_HEIGHT_ROWS:
      hildon_calendar_set_detail_height_rows (calendar,
                                           g_value_get_int (value));
      break;
    case PROP_MIN_YEAR:
      val = g_value_get_int (value);
      if (val <= priv->max_year ||
          val == 0 || priv->max_year == 0)
        {
          priv->min_year = val;
          if (val && (priv->year < val))
            hildon_calendar_select_month (calendar,
                                          priv->month,
                                          priv->min_year);
        }
...
static void
calendar_realize_arrows (HildonCalendar *calendar)
{
  GtkWidget *widget = GTK_WIDGET (calendar);
  HildonCalendarPrivate *priv = calendar->priv;
...
static void
calendar_unrealize_arrows (HildonCalendar *calendar)
{
  HildonCalendarPrivate *priv = calendar->priv;
  gint i;
...
static gint
calendar_get_inner_border (HildonCalendar *calendar)
{
  gint inner_border;
...
static gint
calendar_get_xsep (HildonCalendar *calendar)
{
  gint xsep;
...
static gint
calendar_get_ysep (HildonCalendar *calendar)
{
  gint ysep;
...
hildon_calendar_realize (GtkWidget *widget)
{
  ...
  GdkWindowAttr attributes;
  gint attributes_mask;
  gint inner_border = calendar_get_inner_border (HILDON_CALENDAR (widget));
  GtkAllocation allocation;
  GtkBorder padding;

  get_component_paddings (HILDON_CALENDAR (widget), &padding, NULL, NULL, NULL);
  gtk_widget_get_allocation (widget, &allocation);

  GTK_WIDGET_CLASS (hildon_calendar_parent_class)->realize (widget);
  ...
  calendar_realize_arrows (HILDON_CALENDAR (widget));
}
...
hildon_calendar_unrealize (GtkWidget *widget)
{
  ...
  calendar_unrealize_arrows (HILDON_CALENDAR (widget));
  ...
  GTK_WIDGET_CLASS (hildon_calendar_parent_class)->unrealize (widget);
}

static void
calendar_map_arrows (HildonCalendar *calendar)
{
  HildonCalendarPrivate *priv = calendar->priv;
  gint i;
...
static void
calendar_unmap_arrows (HildonCalendar *calendar)
{
  HildonCalendarPrivate *priv = calendar->priv;
  gint i;
...
static void
hildon_calendar_map (GtkWidget *widget)
{
  HildonCalendarPrivate *priv = HILDON_CALENDAR (widget)->priv;

  GTK_WIDGET_CLASS (hildon_calendar_parent_class)->map (widget);

  gdk_window_show (priv->main_win);

  calendar_map_arrows (HILDON_CALENDAR (widget));
}

static void
hildon_calendar_unmap (GtkWidget *widget)
{
  HildonCalendarPrivate *priv = HILDON_CALENDAR (widget)->priv;

  calendar_unmap_arrows (HILDON_CALENDAR (widget));

  gdk_window_hide (priv->main_win);

  GTK_WIDGET_CLASS (hildon_calendar_parent_class)->unmap (widget);
}

static gchar*
hildon_calendar_get_detail (HildonCalendar *calendar,
                         gint         row,
                         gint         column)
{
  HildonCalendarPrivate *priv = HILDON_CALENDAR (calendar)->priv;
  gint year, month;
...
static gboolean
hildon_calendar_query_tooltip (GtkWidget  *widget,
                            gint        x,
                            gint        y,
                            gboolean    keyboard_mode,
                            GtkTooltip *tooltip)
{
  ...
  if (col != -1 && row != -1 &&
      ...
      0 == (priv->display_flags & HILDON_CALENDAR_SHOW_DETAILS)))
    {
      detail = hildon_calendar_get_detail (calendar, row, col);
  ...
  if (GTK_WIDGET_CLASS (hildon_calendar_parent_class)->query_tooltip)
    return GTK_WIDGET_CLASS (hildon_calendar_parent_class)->query_tooltip (widget, x, y, keyboard_mode, tooltip);
                           ...
                           GtkRequisition *requisition)
{
  ...
  gboolean show_week_numbers = (priv->display_flags & HILDON_CALENDAR_SHOW_WEEK_NUMBERS);
  ...
  if (priv->display_flags & HILDON_CALENDAR_SHOW_HEADING)
    {
      priv->max_month_width = 0;
      for (i = 0; i < 12; i++)
        {
  ...
  priv->max_label_char_ascent = 0;
  priv->max_label_char_descent = 0;
  if (priv->display_flags & HILDON_CALENDAR_SHOW_DAY_NAMES)
    for (i = 0; i < 7; i++)
      {
  ...
  if (priv->detail_func && (priv->display_flags & HILDON_CALENDAR_SHOW_DETAILS))
    {
      gchar *markup, *tail;
      ...
      if (!priv->detail_width_chars || !priv->detail_height_rows)
        for (r = 0; r < 6; r++)
          for (c = 0; c < 7; c++)
            {
              gchar *detail = hildon_calendar_get_detail (calendar, r, c);
...
static void
hildon_calendar_get_preferred_width (GtkWidget *widget,
                                  gint      *minimum,
                                  gint      *natural)
{
  GtkRequisition requisition;

  hildon_calendar_size_request (widget, &requisition);

  *minimum = *natural = requisition.width;
}

static void
hildon_calendar_get_preferred_height (GtkWidget *widget,
                                   gint      *minimum,
                                   gint      *natural)
{
  GtkRequisition requisition;
...
static void
calendar_paint_header (HildonCalendar *calendar, cairo_t *cr)
{
  GtkWidget *widget = GTK_WIDGET (calendar);
  HildonCalendarPrivate *priv = calendar->priv;
  GtkAllocation allocation;
...
static void
calendar_paint_day_names (HildonCalendar *calendar,
                          cairo_t     *cr)
{
  GtkWidget *widget = GTK_WIDGET (calendar);
  HildonCalendarPrivate *priv = calendar->priv;
...
static void
calendar_paint_week_numbers (HildonCalendar *calendar,
                             cairo_t     *cr)
{
  GtkWidget *widget = GTK_WIDGET (calendar);
  HildonCalendarPrivate *priv = calendar->priv;
...
static void
calendar_invalidate_day_num (HildonCalendar *calendar,
                             gint         day)
{
  HildonCalendarPrivate *priv = calendar->priv;
  gint r, c, row, col;
...
static void
calendar_invalidate_day (HildonCalendar *calendar,
                         gint         row,
                         gint         col)
{
  GdkRectangle day_rect;
...
static void
calendar_paint_day (HildonCalendar *calendar,
                    cairo_t     *cr,
                    gint         row,
                    gint         col)
{
  ...
  show_details = (priv->display_flags & HILDON_CALENDAR_SHOW_DETAILS);
...
static void
calendar_paint_main (HildonCalendar *calendar,
                     cairo_t     *cr)
{
  gint row, col;
...
static void
calendar_invalidate_arrow (HildonCalendar *calendar,
                           guint        arrow)
{
  HildonCalendarPrivate *priv = calendar->priv;
...
static void
calendar_paint_arrow (HildonCalendar *calendar,
                      cairo_t     *cr,
                      guint        arrow)
{
  GtkWidget *widget = GTK_WIDGET (calendar);
  ...
  if (((priv->min_year && priv->year <= priv->min_year) ||
       (priv->max_year && priv->year >= priv->max_year)))
    {
      if (priv->min_year && priv->year <= priv->min_year)
        {
          if ((arrow == ARROW_YEAR_LEFT) || (arrow == ARROW_MONTH_LEFT && !priv->month))
            state |= GTK_STATE_FLAG_INSENSITIVE;
        }
      else if (priv->max_year && priv->year >= priv->max_year)
        {
          if ((arrow == ARROW_YEAR_RIGHT) || (arrow == ARROW_MONTH_RIGHT && priv->month == 11))
            state |= GTK_STATE_FLAG_INSENSITIVE;
        }
    }
...
static gboolean
hildon_calendar_draw (GtkWidget *widget,
                   cairo_t   *cr)
{
  HildonCalendar *calendar = HILDON_CALENDAR (widget);
  HildonCalendarPrivate *priv = calendar->priv;
...
static void
calendar_arrow_action (HildonCalendar *calendar,
                       guint        arrow)
{
  switch (arrow)
    {
...
static void
calendar_start_spinning (HildonCalendar *calendar,
                         gint         click_child)
{
  HildonCalendarPrivate *priv = calendar->priv;
...
static void
calendar_stop_spinning (HildonCalendar *calendar)
{
  HildonCalendarPrivate *priv = calendar->priv;
...
static void
calendar_main_button_press (HildonCalendar    *calendar,
                            GdkEventButton *event)
{
  GtkWidget *widget = GTK_WIDGET (calendar);
  HildonCalendarPrivate *priv = calendar->priv;
...
/*      if ((priv->year == private_data->min_year &&
           priv->month == 0 &&
           priv->day_month[row][col] == MONTH_PREV) ||
           priv->year == private_data->max_year &&
           priv->month == 11 &&
           priv->day_month[row][col] == MONTH_NEXT))
        {}*/
  ...
  switch (event->keyval)
    ...
    case GDK_KEY_Left:
      ...
      if (event->state & GDK_CONTROL_MASK)
      ...
      else
        {
          ...
          if (priv->year == priv->min_year && priv->month == 0 &&
              priv->day_month[old_focus_row][old_focus_col-1] == MONTH_PREV) 
            {
    ...
    case GDK_KEY_Right:
      ...
      if (event->state & GDK_CONTROL_MASK)
        ...
        {
          /* if we are at the first allowed day of the minimum year/month then do nothing */
          if (priv->year == priv->max_year && priv->month == 11 &&
              priv->day_month[old_focus_row][old_focus_col+1] == MONTH_NEXT)
            {
              g_signal_emit (calendar, hildon_calendar_signals[ERRONEOUS_DATE_SIGNAL], 0);
              break;
            }
...
static void
hildon_calendar_state_flags_changed (GtkWidget     *widget,
                                  GtkStateFlags  previous_state)
{
  HildonCalendar *calendar = HILDON_CALENDAR (widget);
  HildonCalendarPrivate *priv = calendar->priv;
 ...
 *
 * Returns: a newly #HildonCalendar widget
 **/
GtkWidget*
                                  ...
                                  HildonCalendarDisplayOptions flags)
{
  ...
  if (gtk_widget_get_realized (widget))
    {
      if ((flags ^ priv->display_flags) & HILDON_CALENDAR_NO_MONTH_CHANGE)
        {
          resize ++;
          if (! (flags & HILDON_CALENDAR_NO_MONTH_CHANGE)
              && (priv->display_flags & HILDON_CALENDAR_SHOW_HEADING))
            {
              priv->display_flags &= ~HILDON_CALENDAR_NO_MONTH_CHANGE;
      ...
      if ((flags ^ priv->display_flags) & HILDON_CALENDAR_SHOW_HEADING)
        {
          resize++;
      ...
      if ((flags ^ priv->display_flags) & HILDON_CALENDAR_SHOW_DAY_NAMES)
        {
          resize++;
      ...
      if ((flags ^ priv->display_flags) & HILDON_CALENDAR_SHOW_WEEK_NUMBERS)
        {
          resize++;

          if (flags & HILDON_CALENDAR_SHOW_WEEK_NUMBERS)
            priv->display_flags |= HILDON_CALENDAR_SHOW_WEEK_NUMBERS;
        }

      if ((flags ^ priv->display_flags) & HILDON_CALENDAR_SHOW_DETAILS)
        resize++;
  ...
  g_object_freeze_notify (G_OBJECT (calendar));
  if ((old_flags ^ priv->display_flags) & HILDON_CALENDAR_SHOW_HEADING)
    g_object_notify (G_OBJECT (calendar), "show-heading");
  if ((old_flags ^ priv->display_flags) & HILDON_CALENDAR_SHOW_DAY_NAMES)
    g_object_notify (G_OBJECT (calendar), "show-day-names");
  if ((old_flags ^ priv->display_flags) & HILDON_CALENDAR_NO_MONTH_CHANGE)
    g_object_notify (G_OBJECT (calendar), "no-month-change");
  if ((old_flags ^ priv->display_flags) & HILDON_CALENDAR_SHOW_WEEK_NUMBERS)
                           ...
                           guint        year)
{
  ...
  g_signal_emit (calendar, hildon_calendar_signals[MONTH_CHANGED_SIGNAL], 0);
}
                         ...
                         guint        day)
{
  ...
  g_signal_emit (calendar, hildon_calendar_signals[DAY_SELECTED_SIGNAL], 0);
}
...
/**
 * hildon_calendar_get_day_is_marked:
 * @calendar: a #HildonCalendar
 ...
 */
gboolean
hildon_calendar_get_day_is_marked (HildonCalendar *calendar,
                                guint        day)
{
  HildonCalendarPrivate *priv;
 ...
 *     1 and 31), or %NULL
 *
 * Obtains the selected date from a #HildonCalendar.
 */
void
...
/**
 * hildon_calendar_set_detail_func:
 * @calendar: a #HildonCalendar.
 ...
 * information is shown below each day when #HildonCalendar:show-details is set.
 * A tooltip containing with full detail information is provided, if the entire
 * text should not fit into the details area, or if #HildonCalendar:show-details
 * is not set.
 *
 * The size of the details area can be restricted by setting the
 * #HildonCalendar:detail-width-chars and #HildonCalendar:detail-height-rows
 * properties.
 *
 * Since: 2.14
 */
void
hildon_calendar_set_detail_func (HildonCalendar           *calendar,
...
/**
 * hildon_calendar_set_detail_width_chars:
 * @calendar: a #HildonCalendar.
 * @chars: detail width in characters.
 *
 * Updates the width of detail cells.
 * See #HildonCalendar:detail-width-chars.
 *
 * Since: 2.14
 */
void
hildon_calendar_set_detail_width_chars (HildonCalendar *calendar,
                                     gint         chars)
{
  HildonCalendarPrivate *priv;
...
/**
 * hildon_calendar_set_detail_height_rows:
 * @calendar: a #HildonCalendar.
 * @rows: detail height in rows.
 *
 * Updates the height of detail cells.
 * See #HildonCalendar:detail-height-rows.
 *
 * Since: 2.14
 */
void
hildon_calendar_set_detail_height_rows (HildonCalendar *calendar,
                                     gint         rows)
{
  HildonCalendarPrivate *priv;
...
/**
 * hildon_calendar_get_detail_width_chars:
 * @calendar: a #HildonCalendar.
 *
...
/**
 * hildon_calendar_get_detail_height_rows:
 * @calendar: a #HildonCalendar.
 *
