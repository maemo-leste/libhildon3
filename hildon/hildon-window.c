...
static void
hildon_window_destroy                           (GtkWidget *widget);
                                                 ...
                                                 gint      *natural_width)
{
  ...
  hildon_window_size_request (widget, &requisition);
...
hildon_window_destroy                           (GtkWidget *widget)
{
    ...
    menu_list = g_list_copy (gtk_menu_get_for_attach_widget (GTK_WIDGET (widget)));
    menu_node = menu_list;
    ...
    gtk_widget_set_events (GTK_WIDGET(widget), 0);

    GTK_WIDGET_CLASS (hildon_window_parent_class)->destroy (widget);
}
...
hildon_window_escape_timeout                    (gpointer data)
{
    ...
    ((GdkEventAny *)event)->window = GDK_WINDOW (g_object_ref (gtk_widget_get_window (GTK_WIDGET(data))));
